...
4. **Plain language — the grandmother test.** Write like a bank writes its consumer FAQ: for a reader with no technology and no finance background. Short sentences. Everyday comparisons (a record book, a mailbox key, a receipt) instead of technical terms — never enter developer territory (no protocol/node/hash/consensus vocabulary; if the technical concept *is* the question, explain it with an everyday comparison). Cut every phrase that says nothing. If a sentence needs re-reading, rewrite it.
